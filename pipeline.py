'''
Class to clean football data and upsert it to RDS on AWS

- assume that the source data is already available locally
...
class FootballData():
    ...
    def outcome(result):
        ...
        goals_result = FootballData.goals(result)
    ...
    def __init__(self, folder):
        self.source_data_folder = folder

    def process_data(self):
        print('loading and joining data')
        self._load_data()
        self._join_match_data()
        self._join_team_data()
        self._join_elo_data()
        print('features...')
        self._promo_releg()
        self._cumulative_goals()
        self._streaks()
        self._feature_cleaning()
    ...
    def _load_data(self):
        ...
        league_list = glob.glob(self.source_data_folder)
        ...
        self.league_list = league_list.copy()
        ...
        df_full['outcome'] = df_full['result'].apply(FootballData.outcome)
        self.df = df_full.copy()

    def _join_match_data(self):
        print('joining additional match data')
        ...
        df_teams = self.df['home_team'].unique()
        ...
        self.df['link'] = self.df.apply(lambda row: '/'.join(['','match',team_name_for_link(row['home_team']),team_name_for_link(row['away_team']),str(row['season'])]).strip().lower().replace(' ','-'), axis=1)
        self.df.drop_duplicates(subset=['home_team','away_team','season','league'], inplace=True)

        # join match data with main results df 
        # (df2 in original code)
        self.df = pd.merge(match_df, self.df, how="inner", on='link')

    def _join_team_data(self):
        print('joining additional team data')
        ...
        link_team_data = set(self.df.home_team_x)
        ...
        self.df = pd.merge(self.df, team_df, how="inner", left_on="home_team_x", right_on="matched_team")

    def _join_elo_data(self):
        import pickle
        ...
        # seems like this isn't needed? double-check...
        # df_teams = set(self.df.home_team_x)
        ...
        df4 = pd.merge(self.df, elo, how="left", on="link")
        ...
        print('Before dropping ELO-less matches:',df4.size)
        df4.dropna(subset=['elo_home','elo_away'],inplace=True)
        print('After dropping ELO-less matches:',df4.size)
        df4.sort_values('date_dt',inplace=True)

        # the result so far
        self.df = df4.copy()

    def _promo_releg(self):
        print('adding feature for newly promoted / relegated teams')
        fulldf = self.df.copy()
        ...
        self.seasons = seasons.copy()
        self.df = fulldf.copy()

    def _cumulative_goals(self):
        print('cumulative goals feature')
        self.df[['home_goals_f_cum','home_goals_a_cum']] = self.df.groupby(by=['season','home_team']).cumsum()[['home_goals','away_goals']]
        self.df[['away_goals_a_cum','away_goals_f_cum']] = self.df.groupby(by=['season','away_team']).cumsum()[['home_goals','away_goals']]

    def _streaks(self):
        print('streaks feature')
        df5 = self.df.copy().set_index(['season','league']).sort_values('date_dt')
        ...
        # not sure what it is but it really could do with sorting - although I guess it's only about 10 minutes...
        ...
        self.df = df6.merge(merged_streaks_df, on='link').copy()

    def _feature_cleaning(self):
    ...
    def _write_rds(self):
        ...
        self.df.to_sql('football', engine, if_exists='replace')


if __name__ == '__main__':
    pipeline = FootballData('/Users/dev/aicore/Football-predictor/Football-Dataset/*')
